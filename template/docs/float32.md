...
nav_exclude: true
---
*[conversion-functions](conversion-functions) float32*

## concept

1.  float32 a 32 bits (4 bytes) floating point [value-type](value-type).
2.  float32() is a function converting [data-item](data-item) or [unit](unit) of other value types to the float32 value type.
...
float32(*a*) results in a **32 bits(4 bytes) floating point** [tree-item](tree-item) converted from item *a*. The function results for:

- integer data items, the integer value is interpreted as floating point value (e.g. an integer value 1 is interpreted as 1.0), if the value exceeds the maximum value for the float32 value type, the result is [null](null);
